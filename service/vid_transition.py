...
class DataHandler:
    ...
    def verify_arguments(self, in_args, in_tmp_path):
        ...
        self.temp_path = pathlib.Path(in_args.temp_path)
        ...
        self.phase1_vid = self.temp_path / (self.output.stem + "_phase1_" + timestamp + _OUTPUT_VIDEO_TYPE)
        self.phase2_vid = self.temp_path / (self.output.stem + "_phase2_" + timestamp + _OUTPUT_VIDEO_TYPE)
        self.merged_vid = self.temp_path / (self.output.stem + "_merged_" + timestamp + _OUTPUT_VIDEO_TYPE)
    ...
    def _setup_logging(debug, log_file_path):
        ...
        # Xóa tất cả các handler hiện có để tránh lặp lại
        logger.handlers = []
        ...
        # Thêm file handler nếu ở chế độ debug
        if debug:
            file_handler = logging.FileHandler(str(log_file_path), encoding='utf8')
            file_formatter = logging.Formatter('[%(levelname)s] [%(asctime)s] - %(message)s', "%H:%M:%S")
            file_handler.setFormatter(file_formatter)
            file_handler.setLevel(logging.DEBUG)
            logger.addHandler(file_handler)
            
        # Ngăn chặn việc truyền log đến logger cha
